...
import { Users, RefreshCw, AlertCircle, MailIcon, WifiOff } from 'lucide-react';
...
export const EmployeeTable: React.FC<EmployeeTableProps> = ({
  ...
  return (
    <section className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm flex flex-col min-w-0 overflow-hidden">
      ...
      {/* Banner de desconexión: sin internet las peticiones se pausan (no fallan),
          así que TanStack no entra en error. Se avisa y se explica por qué se ve
          lo que se ve: datos guardados del último fetch, o nada si el filtro
          actual nunca se había consultado. */}
      {isOffline && !isLoading && (
        <div className="mx-5 mt-3 p-2.5 bg-amber-50 border border-amber-200 text-amber-700 rounded-lg text-xs flex items-center gap-2">
          ...
          <span>
            {currentPageEmployees.length > 0
              ...
              : 'Sin conexión: no se pudieron cargar los resultados del filtro actual. Se recuperará al volver.'}
          </span>
        </div>
      )}
        ...
        {isLoading ? (
          ...
          isOffline ? (
            <div className="flex flex-col items-center justify-center gap-2 py-20 text-slate-400">
              ...
              <p className="text-xs text-slate-400">Este filtro no se había cargado antes de perder la conexión.</p>
            </div>
          ) : (
            <div className="flex flex-col items-center justify-center gap-2 py-20 text-slate-400">
