...
import { FileText, FileCheck2, Loader2, AlertCircle, RefreshCw, Timer } from 'lucide-react';
...
export const ReportCard: React.FC = () => {
  ...
  const handleGenerate = () => {
    ...
    setExecutionId(null);
    generate(undefined, {
      onSuccess: (response) => setExecutionId(response.executionId)
    });
  };
  ...
  // Sin job y con error (p. ej. el job desapareció de la API): estado de fallo
  const jobFailed = !!error && !job;
  // Job en procesamiento (incluye la primera consulta de estado en vuelo)
  const isProcessing = (job?.status === REPORT_STATUS.PROCESSING || isLoading) && !timedOut;
  ...
  return (
    <section className="w-80 shrink-0 bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex flex-col gap-4 self-start">
      ...
      {/* Estado: timeout → el job nunca completó a tiempo */}
      {timedOut && job?.status !== REPORT_STATUS.COMPLETED && (
      ...
      {job?.status === REPORT_STATUS.COMPLETED && (
        <div className="flex flex-col gap-3">
